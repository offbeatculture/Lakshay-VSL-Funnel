...
export default function VSL() {
  ...
  // so reloads don't reset the timer (better UX, harder to skip ahead).
  useEffect(() => {
  ...
  function handleBookClick() {
    ...
    startPayment({
      lead: getLead(),
      // 'checkout' mode lands here; 'link' mode leaves the page entirely
      // and Razorpay's callback URL brings users back to /#/schedule.
      ...
      <main className="flex-1">
        <section className="mx-auto max-w-content px-5 pt-8 pb-10 lg:pt-12">
          <div className="text-center">
            ...
            <p className="mt-3 text-sm text-brand-cream/70 sm:text-base">{t.vsl.sub}</p>
          </div>
          ...
          <div className="mx-auto mt-8 max-w-3xl">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm sm:p-8">
              <h2 className="font-display text-xl font-bold text-white sm:text-2xl">{t.vsl.ctaTitle}</h2>
              <p className="mt-2 text-sm text-brand-cream/70">{t.vsl.ctaSub}</p>
              ...
              <div className="mt-5">
                {ctaReady ? (
                  ...
                  >
                    {paying ? 'Opening secure payment…' : `${t.vsl.cta} — Pay Rs.1,000`}
                  ...
                  <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-brand-cream/70">
                    <svg className="h-5 w-5 animate-spin text-brand-orange" viewBox="0 0 24 24" fill="none" aria-hidden>
              ...
              <div className="mt-5 flex items-start gap-3 rounded-lg bg-brand-orange/10 px-4 py-3 text-xs text-brand-cream/85 ring-1 ring-brand-orange/30">
                <svg className="mt-0.5 h-4 w-4 flex-shrink-0 text-brand-orange" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm.75-11.25a.75.75 0 10-1.5 0v4.5c0 .414.336.75.75.75h3a.75.75 0 000-1.5h-2.25v-3.75z" clipRule="evenodd" />
                </svg>
